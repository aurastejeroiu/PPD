...
namespace Lab_4
{
    public static class TaskExecution
    {
        ...
        private static void BeginDownload(string host, int id)
        {
            ...
            StartConnect(requestSocket).Wait(); // connect to remote server
            StartSend(requestSocket, Util.GetRequestString(requestSocket.HostName, requestSocket.Endpoint))
                .Wait(); // request data from server
            StartReceive(requestSocket).Wait(); // receive server response
        ...
        private static Task StartConnect(Content state)
        {
            state.Socket.BeginConnect(state.RemoteEndPoint, AfterConnect, state);

            return Task.FromResult(state.ConnectionDone.WaitOne()); // block until signaled
        }
        ...
        private static Task StartSend(Content content, string data)
        {
            var byteData = Encoding.ASCII.GetBytes(data);
            content.Socket.BeginSend(byteData, 0, byteData.Length, 0, AfterSend, content);

            return Task.FromResult(content.SendingDone.WaitOne());
        }
        ...
        private static Task StartReceive(Content content)
        {
            // receive data
            content.Socket.BeginReceive(content.Buffer, 0, Content.BufferSize, 0, AfterReceive, content);

            return Task.FromResult(content.ReceivingDone.WaitOne());
        }
